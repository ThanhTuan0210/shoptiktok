...
import { ShoppingCart, Search, X, CheckCircle, Zap, Star, ShieldCheck, Settings, ChevronLeft, ChevronRight } from "lucide-react";
...
// Fallback images for any missing products
const FALLBACKS = [
...
function getGallery(product: Product, fallbackIdx: number): string[] {
  ...
  // If only 1 image, add some extras from fallback for demo
  if (all.length < 2) {
    all.push(FALLBACKS[(fallbackIdx + 1) % FALLBACKS.length]);
    all.push(FALLBACKS[(fallbackIdx + 2) % FALLBACKS.length]);
  }
  return all;
}
...
export default function Storefront() {
  ...
  return (
    <div className="bg-[#f5f5f5] text-gray-900 min-h-screen font-sans pb-10 relative">
      ...
      <button onClick={() => setIsAdminOpen(true)} className="fixed bottom-6 left-6 z-40 bg-gray-900 text-white p-3 rounded-full shadow-xl hover:bg-black transition-all group flex items-center" title="Cài đặt giao diện Shop">
        <Settings size={22} />
        ...
        <div className="fixed inset-0 z-[70] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm" onClick={() => setIsAdminOpen(false)}>
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-5">
              <h2 className="text-xl font-bold text-gray-900 flex items-center gap-2"><Settings className="text-[#fe2c55]" size={20} /> Cài đặt Trang Bán Hàng</h2>
              <button onClick={() => setIsAdminOpen(false)}><X size={20} className="text-gray-500" /></button>
            </div>
      ...
      <header className="bg-white sticky top-0 z-40 shadow-sm">
        <div className="max-w-6xl mx-auto px-4 h-16 flex items-center justify-between gap-4">
          ...
          <div className="flex-1 max-w-2xl bg-[#f1f1f1] h-10 rounded-full flex items-center px-4 border border-transparent focus-within:border-[#fe2c55] focus-within:bg-white transition-colors">
            ...
            <input type="text" placeholder="Tìm kiếm sản phẩm..." className="bg-transparent border-none outline-none w-full text-sm text-gray-900 placeholder-gray-500 ml-2" />
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
          ...
          <div className="hidden md:flex flex-col gap-4">
            <div className="flex-1 rounded-2xl bg-white p-4 shadow-sm border border-red-100 flex items-center gap-4">
              ...
              <div><h3 className="font-bold text-gray-900">Flash Sale</h3><p className="text-sm text-gray-600">Giảm giá chớp nhoáng mỗi ngày</p></div>
            </div>
            <div className="flex-1 rounded-2xl bg-white p-4 shadow-sm border border-red-100 flex items-center gap-4">
              ...
              <div><h3 className="font-bold text-gray-900">Mall Chính Hãng</h3><p className="text-sm text-gray-600">Cam kết chất lượng 100%</p></div>
            </div>
          </div>
        </div>
              ...
              <div key={product.id} onClick={() => setSelectedProduct(product)} className="bg-white rounded-xl overflow-hidden shadow-sm hover:shadow-md transition-shadow cursor-pointer group border border-transparent hover:border-[#fe2c55]/30 flex flex-col">
                ...
                <div className="p-3 flex flex-col flex-1">
                  ...
                  <div className="mt-auto">
                    ...
                    <p className="text-[10px] text-gray-500 mt-0.5">Đã bán {Math.floor((product.name.length * 37) % 900) + 100}</p>
                  </div>
                </div>
              </div>
            );
          ...
          <div className="bg-white md:rounded-2xl shadow-2xl w-full h-full md:h-auto md:max-h-[90vh] md:max-w-4xl flex flex-col md:flex-row overflow-hidden" onClick={e => e.stopPropagation()}>
                ...
                <div className="flex gap-2 p-3 bg-white border-t border-gray-100">
                  {gallery.map((img, idx) => (
                    <button key={idx} onClick={() => setActiveImageIdx(idx)}
                      className={`w-14 h-14 rounded-lg overflow-hidden border-2 transition-all shrink-0 ${activeImageIdx === idx ? 'border-[#fe2c55] opacity-100' : 'border-gray-200 opacity-70 hover:opacity-100'}`}>
            ...
            <div className="flex-1 p-5 md:p-8 flex flex-col overflow-y-auto relative">
              ...
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="bg-gray-100 text-gray-700 text-[10px] px-2 py-0.5 rounded font-bold uppercase">{shopName}</span>
              </div>
              ...
              <div className="mb-6 flex-1">
                ...
                {!selectedVariant && variants.filter(v => v.productId === selectedProduct.id).length > 0 && (
                  <p className="text-xs text-amber-600 mt-2 font-medium">⚠️ Vui lòng chọn phân loại trước khi mua</p>
                )}
              </div>

              <div className="flex gap-3">
                <button onClick={() => selectedVariant && addToCart(selectedProduct, selectedVariant)}
                  disabled={!selectedVariant}
                  className="flex-1 bg-[#ffecd8] hover:bg-[#ffe0c2] text-[#ff8000] font-bold py-3.5 rounded-xl transition-colors disabled:opacity-50">
                  Thêm vào giỏ
                </button>
                <button
                  onClick={() => { if (selectedVariant) { addToCart(selectedProduct, selectedVariant); setSelectedProduct(null); setIsCheckoutOpen(true); } }}
                  disabled={!selectedVariant}
                  className="flex-1 bg-[#fe2c55] hover:bg-[#e62045] text-white font-bold py-3.5 rounded-xl shadow-lg shadow-red-200 transition-colors disabled:opacity-50">
                  Mua ngay
                </button>
              </div>
            </div>
        ...
        <div className="fixed inset-0 z-50 flex justify-end bg-black/60 backdrop-blur-sm" onClick={() => setIsCheckoutOpen(false)}>
          <div className="w-full max-w-md bg-white h-full shadow-2xl flex flex-col" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between p-4 border-b border-gray-100">
              <h2 className="text-lg font-bold text-gray-900 flex items-center gap-2"><ShoppingCart size={20} /> Giỏ hàng</h2>
              <button onClick={() => setIsCheckoutOpen(false)} className="p-1 hover:bg-gray-100 rounded-full transition-colors"><X size={22} className="text-gray-700" /></button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-4 bg-[#f9f9f9]">
              {checkoutSuccess ? (
                <div className="text-center py-20 bg-white rounded-xl shadow-sm border border-gray-100">
                  ...
                  <p className="text-sm text-gray-600 mb-6 px-4">Shop sẽ liên hệ xác nhận sớm nhất. Cảm ơn bạn!</p>
                  <button onClick={() => { setIsCheckoutOpen(false); setCheckoutSuccess(false); }} className="px-8 py-2.5 bg-[#fe2c55] text-white rounded-full font-bold text-sm">Tiếp tục mua sắm</button>
                </div>
              ) : cart.length === 0 ? (
                ...
                <>
                    ...
                    <div className="p-4 space-y-4">
                      {cart.map((item, idx) => (
                        <div key={idx} className="flex gap-3">
                          <img src={getImg(item.product, idx)} className="w-20 h-20 object-cover rounded-md border border-gray-100" onError={e => { (e.target as HTMLImageElement).src = FALLBACKS[0]; }} />
                          <div className="flex-1 flex flex-col justify-between">
                            ...
                            <div className="flex items-center justify-between mt-2">
                              ...
                              <span className="text-sm font-bold text-gray-900 bg-gray-100 px-2 py-0.5 rounded">SL: {item.qty}</span>
                            </div>
                          </div>
                        </div>
                      ))}
                  ...
                  <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4 mb-4">
                    ...
                    <input className="w-full text-sm font-medium text-gray-900 placeholder-gray-400 px-4 py-3 bg-white border border-gray-200 focus:border-[#fe2c55] rounded-lg mb-3 outline-none transition-colors" placeholder="Họ và tên người nhận *" value={checkoutForm.name} onChange={e => setCheckoutForm(p => ({ ...p, name: e.target.value }))} />
                    <input type="tel" className="w-full text-sm font-medium text-gray-900 placeholder-gray-400 px-4 py-3 bg-white border border-gray-200 focus:border-[#fe2c55] rounded-lg mb-3 outline-none transition-colors" placeholder="Số điện thoại *" value={checkoutForm.phone} onChange={e => setCheckoutForm(p => ({ ...p, phone: e.target.value }))} />
                    <textarea className="w-full text-sm font-medium text-gray-900 placeholder-gray-400 px-4 py-3 bg-white border border-gray-200 focus:border-[#fe2c55] rounded-lg outline-none transition-colors resize-none h-20" placeholder="Địa chỉ giao hàng *" value={checkoutForm.address} onChange={e => setCheckoutForm(p => ({ ...p, address: e.target.value }))} />
                  </div>
                  ...
                  <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4 space-y-3">
                    ...
                    <div className="flex justify-between text-sm"><span className="text-gray-600 font-medium">Phí vận chuyển</span><span className="font-bold text-green-600">Miễn phí</span></div>
                    <div className="border-t border-gray-100 pt-2 flex justify-between text-base font-bold"><span className="text-gray-900">Tổng cộng</span><span className="text-[#fe2c55]">{formatCurrency(cartTotal)}</span></div>
                  </div>
                </>
              )}
            </div>
            ...
            {!checkoutSuccess && cart.length > 0 && (
              <div className="p-4 bg-white border-t border-gray-200 shadow-[0_-4px_10px_rgba(0,0,0,0.05)]">
                <button onClick={placeOrder} disabled={!checkoutForm.name || !checkoutForm.phone || !checkoutForm.address}
